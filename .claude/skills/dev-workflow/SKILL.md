---
name: "dev-workflow"
description: "Chris's software development workflow — GitFlow branching (main/dev/feature/release/hotfix), commit and PR protocol, clean code patterns, and verification discipline. Use this skill whenever work touches a git repository: before committing, pushing, branching, or opening a PR; when refactoring or cleaning up code; when adding a file, layer, module, or data source to an existing project; when debugging a bug whose root cause isn't yet confirmed; or when reverting. Also trigger on phrases like \"commit this\", \"let's push\", \"clean this up\", \"make a branch\", \"cut a release\", \"why is this still broken\", or when writing code that will live in a repo. Governs the rules that Chris approves every commit and that Chris verifies visual output himself. Consult before any git operation."
---

# Dev Workflow

Chris's engineering workflow: GitFlow branching, clean commits, and code that stays simple as it
grows.

The goal behind all of it is that a repo should be understandable six months from now by someone
who wasn't there — including future Chris. Most of these rules exist because a shortcut that
...
Never run `git commit` without Chris's explicit go-ahead on that specific commit. Not "he said
yes to committing earlier in the session" — each commit needs its own confirmation.

When work is ready, stop and present it: which files changed, what each change does, and a
proposed commit message. Then wait.

This exists because a commit is a claim that something works. Committing before Chris has
confirmed it works turns the git history into a record of guesses, and unwinding those is far
...
### 2. Chris verifies anything visual

Rendered output — map styling, layout, color, spacing, "does this look right" — is Chris's call,
made with his own eyes. Never report a visual problem as fixed based on your own inspection.

The honest formulation is "the change is in, reload and take a look," not "confirmed fixed."

If Chris asks you to verify something visually, that's an invitation to *show him*: capture the
...
- How it was verified — and explicitly, what still needs Chris's eyes
...
1. **Reproduce.** See the actual failure. If you can't observe it, you can't confirm you fixed it.
   When it's visual, get Chris to point at it precisely — location, zoom, what it looks like.
...
- Anything visual → hand to Chris to look at, don't self-certify
...
`Permission denied (publickey)`. Commit locally and tell Chris to push from his own terminal —
